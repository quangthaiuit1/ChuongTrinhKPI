...
@ViewScoped
// KPI ca nhan
public class KPIPersonAssignBean extends AbstractBean<KPIPerson> {
...
//				departmentSearchs.add(member.getDepartment());
		...
		if (validate()) {
			...
			if (kPIPerson.getId() == null) {
				...
				if (kpiPersonOld.isSignResultKPI()) {
					...
					showEdit();
					notify.warning("Phiếu đã duyệt kết quả.");
				} else {
					...
					if (allowUpdate(date) && allowUpdate(date2)) {
						if (kpiPersonOld.getKmonth() != kPIPerson.getKmonth()
							...
							if (temps.size() != 0) {
								...
								if (status) {
									notify.warning("Tháng đã duyệt đăng ký không thay thế được.");
								} else {
									// //////////////////////////////////////////////////////////////////////////////////////////////////////////
									RequestContext context = RequestContext.getCurrentInstance();
									context.execute("PF('dialogConfirm').show();");
	...
	public void showOrientationPerson() {
		...
		// orientationPersons =
		// orientationPersonService.findSearch(positionJobSelect.getCodeDepart(),
		// positionJobSelect);
		// } else {
