...
// KPI phong ban thang
public class KPIDepMonthAssignBLDBean extends AbstractBean<KPIDepOfMonth> {
		...
		if (kpiDepMonth.getId() != null) {
			...
			if (allowUpdate(date)) {
				try {
					...
					wf = kpiDepMonthService.updateSignBLD(wf);
					if (kpiDepMonths.contains(wf)) {
						kpiDepMonths.set(kpiDepMonths.indexOf(wf), wf);
					}
					kpiDepMonths.set(kpiDepMonths.indexOf(wf), wf);
	...
	public void updatesignResultKPIBLD() {
		notify = new Notify(FacesContext.getCurrentInstance());
		if (kpiDepMonth.getId() != null) {
			...
			if (allowUpdate(date)) {
				try {
					...
					wf = kpiDepMonthService.updateSignBLDResult(wf);
					if (kpiDepMonths.contains(wf)) {
						kpiDepMonths.set(kpiDepMonths.indexOf(wf), wf);
					}
					kpiDepMonths.set(kpiDepMonths.indexOf(wf), wf);
	...
	public void createOrUpdate() {
		...
		if (kpiDepMonth.getId() == null) {
			kpiDepMonth.setCodeDepart(member.getDepartment().getCode());
			List<KPIDepMonth> kps = kpiDepMonthService.findKPIDepMonth(kpiDepMonth.getMonth(), kpiDepMonth.getYear(),
					kpiDepMonth.getCodeDepart());
			if (kps.size() == 0) {
				if (allowSave(date)) {
